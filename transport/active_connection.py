...
class ActiveConnection(Connection):
    def __init__(self, host, port, ControllerType = None):
        ...
        self._network_thread = NetworkThread(self._socket, self.on_segment_received)
        self._controller = ControllerType(RawConnection(self._socket, (host, port)))
        self._closing = False
    ...
    def on_tick(self):
        ...
        if not self._closing:
            self._timer = threading.Timer(NETWORK_TICK_SECONDS, self.on_tick).start()
