...
class Segment:
    ...
    def from_datagram(raw_data: bytes):
        ...
        opcode, payload_size, checksum, sequence_number = struct.unpack(
            "!BHBI", header)
        segment = Segment(Opcode(opcode), payload,
                          checksum=checksum, payload_size=payload_size)
    ...
    def serialize(self) -> bytes:
        ...
        assert len(segment) <= MAX_UDP_DATAGRAM_SIZE, \
            f"""Segment won't fit in a datagram
            ({len(segment)} >= {MAX_UDP_DATAGRAM_SIZE})"""
        return segment
    ...
    def __repr__(self):
        ...
        return f"""Segment({Opcode(self.opcode)},
         seq={self.sequence_number}, {payload_repr})"""
